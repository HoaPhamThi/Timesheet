...
const Pagination: IPaginationComponent<IPaginationComponentProps> = (props) => {
    ...
    const handleChangePage = (action: 'set' | 'first' | 'previous' | 'next' | 'last', value: number) => {
        let page = value;
    ...
    return (
        <div className={`components__pagination ${className} d-flex flex-wrap justify-content-center align-items-center`}>
            <Button
                onClick={() => {
                    if ((current ?? 1) > 1) handleChangePage('first', current ?? 1);
                }}
                className={(current ?? 1) > 1 ? 'components__pagination_arrow' : ''}
                startIcon={images.ICON_FIRST_ARROW}
                borderColor="gray"
                background="white"
                iconColor="gray"
            />
            ...
            <Button
                ...
                iconColor="gray"
            />
            {totalPage > 8 && (current ?? 1) > 4 && renderPageGroup('first')}
            {renderPage()}
            ...
            <Button
                onClick={() => {
                    if ((current ?? 1) < totalPage) handleChangePage('last', current ?? 1);
                }}
                startIcon={images.ICON_LAST_ARROW}
                borderColor="gray"
                background="white"
                iconColor="gray"
                className={(current ?? 1) < totalPage ? 'components__pagination_arrow' : ''}
